...
export default function ImportPage() {
  ...
  const { data: jobsData, isLoading: jobsLoading } = useQuery({
    ...
    queryFn: () => importAPI.listJobs().then(r => r.data),
  })
  ...
  async function handleFileChange(e) {
    ...
    setStage('previewing')
    try {
      const res = await importAPI.animalsPreview(f)
      setPreview(res.data)
      setStage('previewed')
    } catch (err) {
  ...
  async function handleConfirm() {
    ...
    setStage('committing')
    try {
      const res = await importAPI.animalsCommit(file)
      setCommitResult(res.data)
      setStage('committed')
      toast.success(`Imported ${res.data.imported} animal${res.data.imported === 1 ? '' : 's'}`)
  ...
  async function handleTemplate() {
    try {
      const res = await importAPI.animalsTemplate()
      downloadBlob(res.data, 'dusuq_animals_template.xlsx')
    } catch {
  ...
  const rowCols = [
    ...
    {
      key: 'status', label: '', width: 32,
      render: (v) => <RowStatusIcon status={v} />,
    },
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500">Bring animal records in from an existing spreadsheet — Excel first, more formats coming.</p>
        </div>
      ...
      <div className="card space-y-4">
        ...
        {stage === 'idle' && (
          <label className="flex flex-col items-center justify-center gap-2 border-2 border-dashed border-gray-200 rounded-xl py-10 cursor-pointer hover:border-primary-300 hover:bg-primary-50/30 transition">
            ...
            <span className="text-xs text-gray-400">Any column layout is fine — we match common header names automatically.</span>
